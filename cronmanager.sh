...
PLUGINDIR=/usr/lib/enigma2/python/Plugins/SystemPlugins
...
;;
   ...
   echo
   if [ -f /bin/enigma ]; then
      echo "Crond Is Automatically Enabled For Enigma1 - Maybe Reboot"
      cp /var/etc/cron/init /var/etc/init
      rm -r /usr/lib/enigma2/python/Plugins/SystemPlugins/Cronmanager > /dev/null 2>&1
      exit 0
   fi
   if [ -f /bin/neutrino ]; then
      echo "Enabling Crond Starting On Neutrino - Reboot To Start Dameon"
...
;; 
   ...
   if [ -f /bin/enigma ]; then
      ...
      echo "Be Disabled And Cronmanager De-Installed"
      echo "-----------------------------------------------------"
      sleep 5
      init 4 > /dev/null 2>&1
      sleep 5
   ...
   echo "-----------------------------------------------------"
   echo "De-Installation Of Cronmanager Finished !"
   echo "-----------------------------------------------------"
   exit 0
;;
"delay")
if  [  ! $2 ]; then
     echo "No Command Passed, Can't Do Anything - Exiting !"
     echo "-----------------------------------------------------"
     exit 1
else
    CRONCOMMAND=$2
...
"add")
if [ ! $2 ]; then
   echo "No Script Passed, Can't Do Anything - Exiting !"
   exit 1
else
  CRONCOMMAND=$2
  if [ ! -f $CRONCOMMAND ]; then
