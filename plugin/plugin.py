...
cronmanager_readme = "/usr/lib/enigma2/python/Plugins/SystemPlugins/Cronmanager/readme.txt"
...
def main(session,**kwargs):
    try:    
        ...
        print "[CRONMANAGER] Plugin Execution Failed"
...
def Plugins(**kwargs):
    return PluginDescriptor(
        name=_("Cron Manager"), 
        description=_("Plugin To Do Cron Management"), 
...
class Cronmanager(Screen):
    skin = """
        <screen position="100,100" size="500,400" title="Cron Manager">
    ...
    def __init__(self, session, args = 0):
        ...
        list.append((_("Re-load Crontab"), "reload"))        
        list.append((_("Show If Cron Daemon Is Running"), "info"))        
    ...
    def go(self):
        ...
        if returnValue is not None:
           if returnValue is "addwizzard":
             ...
             self.session.open(Console,_("Re-Loading Crontab"),["%s reload" % cronmanager_script])
             ...
             self.session.open(Console,_("Readme"),["cat %s" % cronmanager_readme])
             ...
             self.session.open(MessageBox,_("Cron Manager %s By rossi2000" % cronmanager_pluginversion), MessageBox.TYPE_INFO)
...
class AddCommandWizzard(Screen):
    ...
    def askForCommand(self):
	       self.session.openWithCallback(self.processingCommand,InputBox, title=_("Enter a script to execute at a certain time"), text="/etc/cron/scripts/reb.sh 00 4" , maxSize=False, type=Input.TEXT)
...
class LaterCommandWizzard(Screen):
    ...
    def askForLaterCommand(self):
	       self.session.openWithCallback(self.processingLaterCommand,InputBox, title=_("Enter a script to execute with delay"), text="/etc/cron/scripts/reb.sh 60" , maxSize=False, type=Input.TEXT)
